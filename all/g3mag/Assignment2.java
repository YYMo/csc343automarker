...
public class Assignment2 {
  ...
  public int getCountriesNextToOceanCount(int oid) {
    try {
      ...
      ps = connection.prepareStatement("SELECT COUNT(cid) AS numofC FROM " + 
           "a2.oceanAccess WHERE oid=" + oid + "" + "GROUP BY oid");
      rs = ps.executeQuery();

      if (rs.next()) {
        return rs.getInt("numofC");
      } else {
        return -1;
      }
    } catch (SQLException e) {
  ...
  public boolean deleteNeighbour(int c1id, int c2id){
   try {
     ...
     ps = connection.prepareStatement("DELETE FROM a2.neighbour where " +
     		"(country="+ c2id + "AND neighbor=" + c1id + ") OR " +
 ...
 public String listCountryLanguages(int cid){
    ...
    try {
      ...
      rs = ps.executeQuery();
      while (rs.next()) {
    	  result += rs.getString("lid") + ":" + rs.getString("lname") 
  ...
  public boolean updateDB(){
    try {
      ps = connection.prepareStatement("CREATE TABLE a2.mostPopulousCountries " +
      		"AS SELECT cid, cname FROM a2.country WHERE population > 1000000 " +
      		"ORDER BY cid");
      ps.execute();
      ps.close();
    } catch (SQLException e) {
